define([
    ...
    $.widget('mage.amLocator', {
        ...
        selectors: {
            ...
            addressSelector: '#keyword-locator',
            searchSelector: '.btn-search-locator',
        ...
        collectParams: function (sortByDistance, isReset) {
            var keyword = '';
            var state = '';
            if ($('#keyword-locator').length > 0) {
                keyword = $('#keyword-locator').val();
            }
            if ($('#state-locator').length > 0) {
                state = $('#state-locator').val();
            }
            if (!keyword && !state) {
                this.latitude = 0;
                this.longitude = 0;
            ...
            if ($('.amlocator-index-index #lat_value').val() !== '0' && $('.amlocator-index-index #lng_value').val() !== '0') {
                return {
                    'keyword': keyword,
                    'state': state,
                    'lat': $('.amlocator-index-index #lat_value').val(),
                    'lng': $('.amlocator-index-index #lng_value').val(),
        ...
        calculateDistance: function (lat, lng) {
            ...
            for (var location in this.options.jsonLocations.items) {
                ...
                var measurementLabel = $.mage.__('km');
            ...
            setTimeout(function () {
                self.geocoder.geocode({ 'address': value }, function (results, status) {
                    if (status == 'OK') {
                        ...
                        $('.amlocator-index-index #lat_value').val(results[0].geometry.location.lat());
                        $('.amlocator-index-index #lng_value').val(results[0].geometry.location.lng());
                        ...
                        if (region) {
                            $('#region_value').val(region.long_name);
                        } else {
                            $('#region_value').val('');
                        }
                        if ($('#region_value').val()) {
                            self.mapContainer.find('.store-list-state').text($('#region_value').val()).append('<span> Stores<span>');
                            self.mapContainer.find('.store-list-state').show();
                        } else {
                            self.mapContainer.find('.store-list-state').text('');
                            self.mapContainer.find('.store-list-state').hide();
                        }
                        return true;
                    } else {
                        $('.amlocator-index-index #lat_value').val('0');
                        $('.amlocator-index-index #lat_value').val('0');
                        return false;
                    }
                });
        ...
        initializeFilter: function () {
            ...
            self.mapContainer.find('#keyword-locator').change(function () {
                var value = self.mapContainer.find('#keyword-locator').val(),
                        regExp = new RegExp("^[A-Z0-9]{8}\\+\\S{2}$");
                /*if (regExp.test(value) === false) {
                 return false;
                 }*/
                ...
                setTimeout(function () {
                    self.geocoder.geocode({'address': value}, function (results, status) {
                        if (status == 'OK') {
                            ...
                            $('.amlocator-index-index #lat_value').val('0');
                            $('.amlocator-index-index #lng_value').val('0');
                            self.makeAjaxCall();
                        }
                    });
                }, 200);
...
//                self.makeAjaxCall();
            ...
            self.mapContainer.find('#state-locator').change(function () {
                self.getIdentifiers($(this));
                self.makeAjaxCall();
            });
        ...
        createRadiusSlider: function () {
            ...
            if (self.options.minRadiusValue <= self.options.maxRadiusValue) {
                ...
                var radiusValueBuffer = '';
                var radiusValueTimer = '';
